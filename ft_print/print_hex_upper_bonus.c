...
/*   Updated: 2025/11/03 20:14:48 by gomar            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libftprintf_bonus.h"

static int	ft_puthex_digits_upper(unsigned int n)
{
	int		count;
	char	*base;
	...
	if (n >= 16)
		count += ft_puthex_digits_upper(n / 16);
...
int	print_hex_upper(unsigned int n, t_flags *flags)
{
	...
	if (n_len > 0)
		count += ft_puthex_digits_upper(n);
	if (flags->minus == 1)
		count += ft_print_padding(padding.space, ' ');
	return (count);
}
